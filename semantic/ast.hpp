...
extern bool letin;            //boolean that indicates if there is a let in statement
extern void yyerror(const char *msg);


inline std::ostream& operator << (std::ostream &out, Datatype t){
  switch(t){
    case TYPE_unit: out << "unit"; break;
...
public:
  ...
  virtual void sem() {}
  virtual void printOn(std::ostream &out) const = 0; //const = 0 means method becomes virtual
                                                     //and every subclass needs it
...
public:
  ...
  void append_id(Id *id){ expr_list.push_back(id); }
...
public:
  ...
  virtual void sem(){
    st.openScope();
    for(Def *d : def_list) d->sem(); //this will not work for now
    st.closeScope();
  }
...
public:
  ...
  Letdef(Id *id_name, ParList *pl, Type *t, Expr *e): idName(id_name), parList(pl), myType(t), expr(e) { constr_counter = 0; }
  //---------
  //Letdef(bool mut, Id *id_name, ParList *pl, Expr *e): Mutable(mut), idName(id_name), parList(pl), expr(e) { constr_counter = 1; }
  //Letdef(bool mut, Id *id_name): Mutable(mut), idName(id_name) { constr_counter = 2; }
  //Letdef(bool mut, CommaExprList *cel): Mutable(mut), comma_expr_list(cel) { constr_counter = 3; }
  //Variable declaration
  Letdef(Id *id_name, Type *t): idName(id_name), myType(t) { constr_counter = 1; }
  //--------
  //Array Declaration
  Letdef(CommaExprList *cel, Type *t): comma_expr_list(cel), myType(t) { constr_counter = 2; }
  //--------
  ...
  virtual void printOn(std::ostream &out) const override{
    ...
    switch(constr_counter){
      case 0:
        out << *idName;
        if(!parList->isEmpty()) out << "(" << *parList << ")";
        out << " : " << *myType << " = " <<  *expr;
        break;
      case 1:
        out << *idName << ": " << *myType;
  ...
  virtual void sem(){
    ...
    switch(constr_counter){
      case 0:
        if(myType->getTypeName() == "unit") st.insert(idName->getIdName(), TYPE_unit);
        else if(myType->getTypeName() == "int") st.insert(idName->getIdName(), TYPE_int);
        else if(myType->getTypeName() == "char") st.insert(idName->getIdName(), TYPE_char);
        else if(myType->getTypeName() == "bool") st.insert(idName->getIdName(), TYPE_bool);
        else if(myType->getTypeName() == "float") st.insert(idName->getIdName(), TYPE_float);
...
public:
  ...
  virtual void printOn(std::ostream &out) const override{
    out << BLUE << "LetIn(" << RESET << *let << BLUE << " in " << RESET << *expr << BLUE << ")" << RESET << endl;
  }
...
public:
  ...
  /*void update_list(Id *id){
    exprList->append_id(id);
    std::cout << "HI";
  }*/
